...
use crossterm::{
    cursor, event::{self, read, KeyEventKind}, style, terminal, ExecutableCommand, QueueableCommand
};
...
fn handle_event(mode: &Mode, stdout: &mut std::io::Stdout, ev: event::Event) -> Result<Option<Action>> {
    match mode {
...
fn main() -> Result<()> {
    ...
    stdout.execute(terminal::EnterAlternateScreen)?; // Enter alternate screen
    ...
    stdout.execute(terminal::LeaveAlternateScreen)?; // Leave alternate screen
